...
from models.resnet import ResnetBlock, ResidualBlock_noBN
...
class KernelWizard(nn.Module):
    def __init__(self, opt):
        ...
        # Features extraction
        resBlock_noBN_f = functools.partial(ResidualBlock_noBN,
                                            nf=nf)
        feature_extractor = []

        feature_extractor.append(nn.Conv2d(num_image_channels, nf, 3, 1, 1,
                                           bias=True))
        ...
        recon_trunk.append(lrelu)
        recon_trunk.append(nn.Conv2d(64, num_image_channels,
                                     3, 1, 1, bias=True))
