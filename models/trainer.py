...
class Trainer(BaseTrainer):
    ...
    def feed_data(self, data, need_GT=True):
        ...
        LQ = tensor2img(self.LQ[0])
        HQ = tensor2img(self.HQ[0])
        cv2.imwrite('LQ.png', LQ)
        cv2.imwrite('HQ.png', HQ)
    ...
    def optimize_parameters(self, step):
        ...
        if self.use_vae:
            KL_divergence = self.l_kl_w * torch.sum(
                torch.pow(kernel_mean, 2) + torch.pow(kernel_sigma, 2)
                - torch.log(1e-8 + torch.pow(kernel_sigma, 2)) - 1
            ).sum()
