...
class MixGrayDataset(data.Dataset):
    '''
    ...
    def __init__(self, opt):
        super(MixGrayDataset, self).__init__()
        self.opt = opt
    ...
    def _init_lmdb(self):
        ...
        for HQ_root, LQ_root in zip(self.HQ_roots, self.LQ_roots):
            self.HQ_envs.append(lmdb.open(HQ_root, readonly=True,
                                            lock=False, readahead=False,
                                            meminit=False))
            self.LQ_envs.append(lmdb.open(LQ_root, readonly=True,
                                            lock=False, readahead=False,
                                            meminit=False))
    ...
    def __getitem__(self, index):
        ...
        for v in neighbor_list:
            img_HQ = util.read_img_gray(self.HQ_envs[env_idx],
                                        '{}_{:08d}'.format(name_a, v),
                                        (3, 720, 1280))
            img_HQ_l.append(img_HQ)

        #### get LQ images
        img_LQ = util.read_img_gray(self.LQ_envs[env_idx],
                                    '{}_{:08d}'.format(name_a, neighbor_list[-1]),
                                    (3, 720, 1280))
        if self.opt['phase'] == 'train':
            C, H, W = 3, 720, 1280  # LQ size
            # randomly crop
